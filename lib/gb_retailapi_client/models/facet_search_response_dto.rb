  ...
  class FacetSearchResponseDto
    ...
    def self.openapi_types
      {
        ...
        :'available_navigation' => :'NavigationDto'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GbRetailapiClient::FacetSearchResponseDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GbRetailapiClient::FacetSearchResponseDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'available_navigation')
        self.available_navigation = attributes[:'available_navigation']
      else
    ...
    def hash
      [original_request, available_navigation].hash
    end
