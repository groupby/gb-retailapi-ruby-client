  ...
  class TemplateDto
    ...
    def self.openapi_types
      {
        :'name' => :'String',
        :'rule_name' => :'String',
        :'trigger_set' => :'TemplateDtoTriggerSet',
        :'zones' => :'Array<ZoneDto>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GbRetailapiClient::TemplateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GbRetailapiClient::TemplateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'trigger_set')
        self.trigger_set = attributes[:'trigger_set']
      end
    ...
    def hash
      [name, rule_name, trigger_set, zones].hash
    end
